...
public class CookieController {
    ...
    public String create(HttpServletResponse response) {
        ...
        // name 및 value 속성을 필수로 작성해줘야 함 + expire 등은 선택
        // name, value 모두 문자열(ASCII 코드)만 가능 vs 한글 쓰고 싶다면, 공공 데이터 수업 때 사용한 unicode encoder? 사용(=번거로움)
...
//		ck.setMaxAge(86400); // 60초 * 60분 * 24시간 = 1일 = 86400초
    ...
    // 2022.3.2(수) 10h_
    // 쿠키 삭제 = 별도 메소드는 없음, 삭제 명령어가 따로 존재하지 않음 -> 브라우저 개발자 도구에서 사용자가 삭제 ou
    // 같은 이름으로 재발급/생성하면 덮어쓰기 됨 + 지속/만료 시간을 0초로 설정해서 발급 = 삭제
